...
def pyramid_score(img):
    ...
    return mask_pyramid(eroded, 3)[1]
...
def create_tile(centroid, tile_size, pixel_size, images, frames, centroids, bounds, v_mask, k_mask, downsample, M):
    ...
    for i in tqdm(IDS, "Estimating Tile Contents", leave=False):
        image = images[i]
        img = np.asarray(Image.open(image)).astype(np.float32)
        c = centroids[i]
        frame = frames[i]
    ...
    # plt.imshow(np.mean(res, axis=-1)); plt.show()
    acc = np.zeros((*pixel_size, 3))
    C = 0
...
def create_map(
    ...
    DSQ = u * u + v * v
    d_mask = np.exp(-DSQ * kernel_strength)


    for T_ID, x in enumerate(tqdm(X)):
        centroid = (x / N + 0.5) * diff + map_min
        tile = create_tile(
            centroid,
            diff / N,
        ...
        if not tile is None:
            dest = str(Path(project).parent / f"tile_{T_ID}.jpg")
